...
// nonCriticalStepCount is how many steps run after transcode (thumbnails, audio,
// preview, streaming). It is the ceiling for MAX_PARALLEL_POST_TRANSCODE_STEPS:
// asking for more than there are steps buys nothing.
const nonCriticalStepCount = 4

// clampParallelSteps bounds MAX_PARALLEL_POST_TRANSCODE_STEPS to [1, nonCriticalStepCount].
func clampParallelSteps(maxParallelSteps int) int {
	if maxParallelSteps < 1 {
		return 1
	}
	if maxParallelSteps > nonCriticalStepCount {
		return nonCriticalStepCount
	}
	return maxParallelSteps
}

// DefaultWorkerCount is the worker count used when WORKER_COUNT is not set.
//
// One worker per core was the original default, on the premise that the worker is
// the unit of parallelism. It is not: a single job runs up to clampParallelSteps
// FFmpeg processes at once (steps 4-7), and no FFmpeg command passes -threads, so
// each process opens roughly 1.5x NumCPU threads on its own. NumCPU workers times
// those processes oversubscribes the host by the same factor.
//
// Dividing by the processes a job can spawn keeps the concurrent FFmpeg count near
// the core count, and follows the two knobs that create the contention: turning
// PARALLEL_NON_CRITICAL_STEPS off, or lowering MAX_PARALLEL_POST_TRANSCODE_STEPS,
// raises the default back up on its own.
func DefaultWorkerCount(numCPU int, parallelSteps bool, maxParallelSteps int) int {
	processesPerJob := 1
	if parallelSteps {
		processesPerJob = clampParallelSteps(maxParallelSteps)
	}

	workers := numCPU / processesPerJob
	if workers < 1 {
		return 1
	}
	return workers
