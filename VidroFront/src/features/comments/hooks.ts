...
type ReactionFields = {
  userReaction: { id: number; value: string } | null
  likeCount: number
  dislikeCount: number
}
...
function withReaction<T extends ReactionFields>(item: T, type: number): T {
  ...
  return {
    ...
    userReaction: {
      id: type,
      value: type === ReactionType.Like ? 'Like' : 'Dislike',
    },
    likeCount:
      item.likeCount +
      (type === ReactionType.Like ? 1 : 0) -
      (prev?.id === ReactionType.Like ? 1 : 0),
    dislikeCount:
      item.dislikeCount +
      (type === ReactionType.Dislike ? 1 : 0) -
      (prev?.id === ReactionType.Dislike ? 1 : 0),
  }
}
...
function withoutReaction<T extends ReactionFields>(item: T): T {
  ...
  return {
    ...
    dislikeCount:
      item.dislikeCount - (prev?.id === ReactionType.Dislike ? 1 : 0),
  }
}
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      if (variables.parentCommentId) {
        ...
        queryClient.invalidateQueries({
          queryKey: commentKeys.list(videoId, 0),
        })
      }
    },
  })
  ...
  return useMutation({
    ...
    onMutate: async ({ commentId, type, parentCommentId }) => {
      if (parentCommentId) {
        ...
        const snapshot =
          queryClient.getQueryData<InfiniteData<RepliesPage>>(key)
          ...
          return {
            ...
            pages: old.pages.map((page) => ({
              ...
              replies: page.replies.map((r) =>
                r.commentId === commentId ? withReaction(r, type) : r,
              ),
            })),
          }
        })
      ...
      const snapshot0 = queryClient.getQueryData<InfiniteData<CommentsPage>>(
        commentKeys.list(videoId, 0),
      )
      const snapshot1 = queryClient.getQueryData<InfiniteData<CommentsPage>>(
        commentKeys.list(videoId, 1),
      )
        ...
        return {
          ...
          pages: old.pages.map((page) => ({
            ...
            comments: page.comments.map((c) =>
              c.commentId === commentId ? withReaction(c, type) : c,
            ),
          })),
        }
      }
        ...
        queryClient.setQueryData<InfiniteData<RepliesPage>>(key, (old) => {
          ...
          return {
            ...
            pages: old.pages.map((page) => ({
              ...page,
              replies: page.replies.map((r) =>
                r.commentId === commentId ? withoutReaction(r) : r,
              ),
            })),
          }
        })
      ...
      const update = (old: InfiniteData<CommentsPage> | undefined) => {
        ...
        return {
          ...
          pages: old.pages.map((page) => ({
            ...page,
            comments: page.comments.map((c) =>
              c.commentId === commentId ? withoutReaction(c) : c,
            ),
          })),
        }
      }
